...
import { call, notify, errorText } from "./rpc.js";
import { state, emit, resetSession } from "./state.js";
...
export async function refreshSessions() {
  try {
    const result = await call("nest.sessions");
...
export function handleNotification(method, params) {
  ...
  if (method === "nest.daemon_event") daemonEvent(params);
}

function daemonEvent(params) {
  switch (params.kind) {
    case "session_evicted":
      if (params.session_id === state.sessionId) {
        banner(t("banner.evicted", { reason: params.reason || "" }));
      }
      refreshSessions();
      break;
    case "mcp_connect_failed":
      ...
      break;
    case "scene_degraded":
      banner(t("banner.sceneDegraded", { scene: params.scene, reason: params.reason || "" }));
      break;
    case "config_reloaded":
      banner(t("banner.configReloaded"));
