...
//!
//! Nest embeds AttaCore's daemon rather than talking to one over a socket
//! (docs/architecture.md §2), so this crate does what `attacored`'s `main.rs`
//! does — resolve scenes, load the three settings tiers, build the model
...
//!
//! It is a copy of an assembly that lives upstream, and the only reason it is
//! a copy is that upstream exposes no bootstrap entry point. If AttaCore ever
//! grows `daemon::bootstrap::build(config)`, this file becomes a call to it.
...
pub struct EngineConfig {
    ...
    /// What `ATTA_CONFIG_HOME` names; Nest resolves it (see
    /// `crates/app/src/paths.rs`) and passes it in rather than letting this
    /// crate read the environment, so one process could serve two roots.
