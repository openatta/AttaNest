...
import { $, el, button, icon, ago, clip } from "../runtime/dom.js";
...
import { openSession, refreshSessions, banner } from "../shell/session.js";
import { newSessionDialog, sessionMenu, addWorkspaceDialog, renameDialog, openModal, closeModal } from "../shell/modals.js";
import { openSettings } from "../shell/settings.js";
import { toggleTheme, themeIsDark } from "../runtime/theme.js";
...
function renderGroups(list) {
  ...
  if (archivedCount) {
    list.appendChild(button("archive-toggle", [
      ...
      // Remembered, because it is a view preference and the store has a place
      // for one. Read without ever being written, `prefs` was a value nobody
      // set — which reads as a working feature and is not one.
      call("nest.prefs.set", { key: "showArchived", value: state.showArchived }).catch(() => {});
      emit("sessions");
    }));
  }
}

/** Ask the `sidebar.group` point how these sessions are grouped.
 *
 * The shell keeps the chrome — collapse, drag, the archived toggle — and the
 * contribution decides membership and order. A contribution that throws loses
 * only the grouping: everything falls into one list rather than the sidebar
 * going blank.
 */
function groupSessions(sessions) {
  const contributions = at("sidebar.group");
  const chosen = contributions[contributions.length - 1];
  if (!chosen) return [{ key: "all", title: t("sidebar.byRecent"), rows: sessions }];
  const groups = renderPoint("sidebar.group", { ...chosen, render: chosen.group }, sessions);
  return groups || [{ key: "all", title: t("sidebar.byRecent"), rows: sessions }];
}
